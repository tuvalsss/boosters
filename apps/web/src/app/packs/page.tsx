...
export default function PacksPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-8 lg:px-8">
      <h1 className="text-3xl font-bold tracking-tight">Open packs</h1>
      <p className="text-sm text-white/55">
        Provably-fair draws. The server seed is committed before you open and revealed after, so any
      ...
      {loading ? (
        ...
        <p className="mt-10 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-10 text-center text-white/40">
          No active packs right now. Check back soon.
        </p>
      ) : (
          ...
          {packs.map((p, i) => (
            ...
            >
              <div className="relative mx-auto h-44 w-28">
                <Image
                  src={BRANCHES[i % BRANCHES.length]!.packImage}
                  alt={p.name}
                  fill
              ...
              <div className="mt-1 flex items-center justify-between text-xs">
                <span className="text-white/45">{p._count?.poolItems ?? 0} left</span>
